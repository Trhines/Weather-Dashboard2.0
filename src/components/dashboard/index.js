      ...
      <Tab key={"+"} eventKey="+" title="+">
        <Container className="searchTab">
          <div className="mainContent bg">
            ...
            <ul>
              {results.map((result) => <Results key={result.name} name={result.name} all={result} country={result.country} temp={result.main.temp} lat={result.coord.lat} lon={result.coord.lon} update={updateData} clear={clearResults} />)}
            </ul>
          </div>
        </Container>
      </Tab>
